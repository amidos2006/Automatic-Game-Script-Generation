...
It uses automated player described in \secref{automatedPlayer} to measure each level score and the best score is reported. Level score is calculated using the same equation in \secref{levelScoreEquation} except for Playability Score where the Do Nothing Score is removed from calculations. The removing of Do Nothing Score is to give equal chance to all Winning Rule Conditions. As No Rule always have Do Nothing Score equal to zero compared to other rules.
...
\begin{itemize} \itemsep0pt \parskip0pt \parsep0pt
	\item \textbf{Player exists score ($P_{score}$):} It ensures that player exists in any of the game rules. It can be expressed using the following equation:
	\begin{center}
		$P_{score}= \begin{cases}
		           ...
		           \end{cases}$
		\end{center}
	\item \textbf{Critical path exists score ($C_{score}$):} It ensures the existence of a path between the player object and at least one of the winning objects in the rules in the Relations list. It can be expressed using the following equation:
	\begin{center}
	$C_{score}= \begin{cases}
	           ...
	           \end{cases}$
	\end{center}
	\item \textbf{Useless objects score ($U_{score}$):} The amount of useless objects found in the game rules. Useless objects are rule objects that are neither connected to a player or a winning condition in the Relations list. The score can be expressed using the following equation:
	\begin{center}
	$U_{score} = \dfrac{1}{N_{useless} + 1}$
	\end{center}
	where $N_{useless}$ is the number of useless objects.
	\item \textbf{Winning condition validity score($W_{score}$):} Winning objects related to winning condition should have some features to ensure game playability. Features differ according the type of Winning condition. No rule requires at least one of winning object having a Delete, Move, or Teleport behavior, while other rules (All and Some) requires Create, Move, or Teleport behavior. This score can be expressed by the following equation:
	\begin{center}
	$W_{score}= \begin{cases}
	               1 & \text{winning object have the required features}\\
	               0 & \text{otherwise}
	           \end{cases}$
	\end{center}	
	\item \textbf{Winning object in rules score ($WO_{score}$):} At least one of the two winning objects appear in game rules. This score can be expressed using the following equation:
	\begin{center}
	$WO_{score}= \begin{cases}
	           ...
	           \end{cases}$
	\end{center}
	\item \textbf{Player in LHS score ($PL_{score}$):} Player should exists on the left hand side of at least one rule. This score can be expressed using the following equation:
	\begin{center}
	$PL_{score}= \begin{cases}
	           ...
	           \end{cases}$
	\end{center}	
	\item \textbf{Player Movement ($M_{score}$):} Player object should have a movement action associated with it to affect other game objects based on his actions. This score can be expressed using the following equation:
	\begin{center}
	$M_{score}= \begin{cases}
	           ...
	           \end{cases}$
	\end{center}	
	\item \textbf{Direction consistency ($D_{score}$):} To have understandable rules, rules should have same movement action across the whole rule. The score is inversely proportional to the number of different movements in the rules. This score can be expressed using the following equation:
	\begin{center}
	$D_{score} = \dfrac{1}{D_{unique}}$
	\end{center}
	where $D_{unique}$ is the average number of unique movement for all rules.
\end{itemize}
\textbf{Rule Validity Score ($V_{score}$):} It ensures no error found in the generated rules. It can be expressed using the following equation:
\begin{center}
